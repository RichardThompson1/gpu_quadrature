module quadrature_module_cpu
  ...
  subroutine quadrature_cpu_serial(x_array, y_array, t_array, result_array, hx, hy, n, m)
    implicit none
  ...
  subroutine quadrature_cpu_parallel(x_array, y_array, t_array, result_array, hx, hy, n, m)
    implicit none
    ...
    !$OMP PARALLEL DO SCHEDULE(DYNAMIC) PRIVATE(i,j,f,w_ij,local_sum)
    do k = 1, size(t_array)
      local_sum = 0.0
      do i = 1, size(x_array)
        do j = 1, size(y_array)
          ...
          f = sin(n * pi * x_array(i)) * sin(m * pi * y_array(j))
          local_sum = local_sum + (w_ij * f) * (hx * hy / 4.0)
        end do
      end do
      result_array(k) = local_sum
    end do
    !$OMP END PARALLEL DO
  end subroutine
